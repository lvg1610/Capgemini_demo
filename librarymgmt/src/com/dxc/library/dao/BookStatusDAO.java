...
public class BookStatusDAO 
{
    ...
    public boolean modifyBookStatus(char status,int bookId)
    {
    	Connection con = null;
    	PreparedStatement pst = null;
    	String qry="update bookstatus set availablecopies=availablecopies-1 where bookid=?";
    	if(status=='r') qry="update bookstatus set availablecopies=availablecopies+1 where bookid=?";
    	int count = 0;
    	try
    	{
    		con = DBConnection.getConnection();
    ...
    public int getAvailableCopies(int bookId)
    {
    	...
    	{
    		con = DBConnection.getConnection();
    		pst = con.prepareStatement("select availablecopies from bookstatus where bookid=?");
    		pst.setInt(1, bookId);
    		rs = pst.executeQuery();
